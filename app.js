...
window.onload = () => {
    ...
    let currentPlayersScores = document.querySelectorAll(".player-current-score");
    ...
    // New Game function
    const newGameModal = document.querySelector(".new-game-modal"); /* New game button */
     ...
     let namePlayer1 = document.getElementById("name-player1"); /* Name 1 in the label */
     let namePlayer2 = document.getElementById("name-player2"); /* Name 2 in the label  */
     let player1 = document.querySelector(".player1-name"); /* Name 1 in the game block */
     let player2 = document.querySelector(".player2-name"); /* Name 2 in the game block */
     
     const playersModal = () => {
       playBtn.addEventListener("click", () => { /* Play button, on click */
         if (namePlayer1.value == "") { /* If no entry name of Player 1 */
           player1_Name = "Player 1"; /* Then enter "Player 1" */
         } else {
           player1_Name = namePlayer1.value; /* Or entry name of Player 1 in the input */
         }
         player1.textContent = player1_Name; /* And add entry name of Player 1 in the game block */
         if (namePlayer2.value == "") { /* If no entry name of Player 2 */
           player2_Name = "Player 2"; /* Then enter "Player 2" */
         } else {
           player2_Name = namePlayer2.value; /* Or entry name of Player 2 in the input */
         }
         player2.textContent = player2_Name; /* And add entry name of Player 2 in the game block */
    ...
    const playDice = () => {
      rollDice.addEventListener("click", () => { /* On roll dice click, it triggers dice animation and dice number */
      ...
      rollDice.onclick = function() { /* On roll dice click, it triggers the dice audio song */
        soundDice.play();
        soundDice.volume = 1;
      }
    ...
    const player = document.querySelectorAll(".player"); /* player's icon, active or not */
    ...
    const modalNewGameAfterWin = document.getElementById("newGameModal-after-win");
    
    const winnerModal = () => { /* Additional variable for winner's modal */
      if (globalScoreP1 >= 100 || globalScoreP2 >= 100) {
        ...
        modalNewGameAfterWin.addEventListener("click", () => { /* New game button in the winner modal, on click */
          winModal.style.display = "none";
          chooseModal.style.display = "block";
          playersModal();
        });
